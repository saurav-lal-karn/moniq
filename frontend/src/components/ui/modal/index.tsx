...
export const Modal: React.FC<ModalProps> = ({
    ...
    return createPortal(
        <div
            className="scrollbar-hide modal fixed inset-0 z-[99999] overflow-x-hidden overflow-y-auto"
            style={{ zIndex: 99999 }}
        >
            {!isFullscreen && (
                <div
                    className="fixed inset-0 h-full w-full cursor-pointer bg-gray-400/50 backdrop-blur-[32px]"
                    onClick={onClose}
                ></div>
            )}
            <div
                className={`flex min-h-full items-center justify-center ${!isFullscreen ? "p-4 sm:p-6 lg:p-8" : ""}`}
            >
                <div
                    ref={modalRef}
